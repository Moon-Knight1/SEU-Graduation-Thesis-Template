...
系统前端提供了对三类缺陷数据表的分页查询功能。由于缺陷表数据量较大，当用户查询较靠后的页码时，普通分页方式的查询效率会明显下降。其原因在于，对于 LIMIT offset, N 语句，数据库需要先扫描 offset + N 条记录，再丢弃前 offset 条数据，最终返回当前页所需的 N 条记录。当偏移量较大时，会产生大量无效扫描；如果查询字段较多，还可能带来较多回表操作，从而影响接口响应速度\cite{botros2021highperformance}。
...
\begin{enumerate}[label=(\arabic*)]	
	...
	\item 
	项目聚类缺陷统计主要用于对多个项目的缺陷数据进行汇总分析，支撑聚类维度的质量看板展示。
	\begin{figure}[htbp]
		\centering
